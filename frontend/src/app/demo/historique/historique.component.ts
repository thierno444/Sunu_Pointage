...
export class HistoriqueComponent implements OnInit {
  ...
  updateAnalytics() {
  const absents = this.filteredPointages.filter(p => !p.estPresent).length;
  const presents = this.filteredPointages.filter(p => p.estPresent).length;
...
onDateChange(): void {
  ...
  this.currentPage = 1;
  // 🔧 AJOUT ICI : recalculer le total après le filtrage
  this.updateAnalytics();
}
