   ...
   public function pointer(Request $request)
{
    ...
    // Heure actuelle au format HH:mm (string)
    $now = date('H:i');
    // Date du jour
    $today = date('Y-m-d');

    // Heure de début pour calcul retard (08:30)
    ...
    // Fonction simple pour comparer heures au format "HH:mm"
    $estEnRetard = ($now > $heureDebutJournee);
    ...
    if ($pointage) {
        ...
        if (!$pointage->premierPointage) {
            $updateData['premierPointage_temp'] = $now;  // stocke "HH:mm"
            $updateData['estRetard_temp'] = $estEnRetard;
        } else {
            $updateData['dernierPointage_temp'] = $now; // stocke "HH:mm"
        }
